...
class TestSigunguDashboardPath:
    """시군구 인구 CSV 가 없어 metrics_sgg.parquet 이 만들어진 적이 없다.

    즉 대시보드의 시군구 경로(접근성 탭, key="zscode")는 상시 무보증 상태였다.
    합성 시군구 지표로 app.py 본문을 실제로 태워 예외가 없는지 확인한다.
    """

    def test_app_renders_with_sigungu_metrics(self, tmp_path, monkeypatch) -> None:
        import os
        import runpy
        import sys
        ...
        try:
            from streamlit import delta_generator  # noqa: PLC0415

            monkeypatch.setattr(delta_generator, "_maybe_print_use_warning", lambda *a, **k: None)
