...
| `App.tsx` | 최상위 화면 컴포넌트. 필터 상태(시도/속도/운영기관/24시간)를 소유하고 KPI·상태 분포·지도·랭킹 논리 영역의 렌더링을 조정 |
...
| `data.test.ts` | 희소 지역·격자·상태 집계와 `totalTerms`의 필터·분모 범위 계약을 검증 |
| `metrics.ts` | 지표 평가기(`evaluate`, `format`, `isVulnerableFirst`, `byId`). `metrics.json`을 읽어 나눗셈만 수행하고 공식 자체는 없다 |
| `metrics.test.ts` | 지표 평가·포맷·취약 방향·ID 조회의 순수 함수 계약을 검증 |
| `selectors.ts` | 랭킹, 지도 취약도, 활성 필터 설명, 0건 복구 가능성, 상태 분포, 운영기관 개수를 만드는 순수 도메인 파생 계층 |
| `selectors.test.ts` | `selectors.ts`의 해상도 강등·미진출 구분·고정 취약도 도메인·필터 복구 계약을 검증 |
| `DistributionMap.tsx` | 시군구 코로플레스(기본), 2km 격자, 히트맵, 밀집 원(비례 원)을 전환하는 MapLibre GL 지도. 조건부 3D는 ADR 0001 여섯 제약을 따르며 밀집 원은 경계·배경지도 실패 중에도 유지 |
| `DistributionMap.module.css` | 지도 스타일 |
| `mapView.ts` | 지도 뷰·fallback 상태에서 레이어 가시성과 사용자 안내 문구를 결정하는 React/MapLibre 비의존 순수 함수 |
...
- `App.tsx`는 상태 연결과 렌더링 조정에 집중한다. 여러 화면 영역이 쓰는 순수 도메인 파생은 `selectors.ts`에 두고 `selectors.test.ts`에서 직접 검증한다.
